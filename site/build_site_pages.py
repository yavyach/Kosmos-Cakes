...
Шаблон страницы торта (один раз): site/templates/cake-page-{prefix,mid,suffix}.html.
Итоговый site/cakes/{id}.html собирается при build (не править вручную).
Общий JS — site/cake-page.js.
...
def get_class_assets_version() -> int:
    """Максимальный mtime среди class-base/classic/lux.svg — cache-bust
    bb-class img src (window.KOSMOS_CLASS_V на странице торта)."""
    v = 0
...
def load_template(name: str) -> str:
    if name not in _TEMPLATE_CACHE:
        _TEMPLATE_CACHE[name] = (SITE / "templates" / name).read_text(encoding="utf-8")
    return _TEMPLATE_CACHE[name]


def cake_asset_manifest() -> dict[str, str]:
    ...
    return {
        ...
        "DLV_CSS_V": dcss_v,
        "DLV_JS_V": djs_v,
        "CLASS_V": str(get_class_assets_version()),
    }


def page_embed_snippet(
    rel_path: str, public_site: str, cakes_by_id: dict | None = None
) -> str:
    """Тело страницы для вставки в Tilda: абсолютные URL на GitHub Pages."""
    if (
        cakes_by_id
        and rel_path.startswith("cakes/")
        and rel_path.endswith(".html")
    ):
        cid = Path(rel_path).stem
        if cid in cakes_by_id:
            text = assemble_cake_page(cakes_by_id[cid])
        else:
            return ""
    else:
        path = SITE / rel_path
...
def fill_head_html(cake: dict) -> str:
    if cake["id"] in PHOTO_GRID_CAKES:
        return ""
    return fill_head_markup()


def cake_page_fields(cake: dict) -> dict[str, str]:
    ...
    return {
        ...
        "THIRD_COL_INNER": third_col_inner_html(cake),
    }


def apply_placeholders(text: str, fields: dict[str, str]) -> str:
    for key, val in fields.items():
        text = text.replace(f"__{key}__", val)
    return text


def apply_manifest(text: str, manifest: dict[str, str] | None = None) -> str:
    m = manifest or cake_asset_manifest()
    for key, val in m.items():
        text = text.replace(f"__{key}__", val)
    return text


def render_cake_body(cake: dict) -> str:
    """Средняя часть страницы торта (три колонки)."""
    return apply_placeholders(load_template("cake-card-body.html"), cake_page_fields(cake))


def assemble_cake_page(cake: dict) -> str:
    """Полная страница (для preview/Tilda) — prefix + content + suffix."""
    fields = cake_page_fields(cake)
    manifest = cake_asset_manifest()
    prefix = apply_manifest(
        apply_placeholders(load_template("cake-page-prefix.html"), fields),
        manifest,
    )
    body = render_cake_body(cake)
    mid = load_template("cake-page-mid.html")
    suffix = apply_manifest(
        apply_placeholders(load_template("cake-page-suffix.html"), fields),
        manifest,
    )
    return prefix + body + mid + suffix
...
def write_site_preview_index(cakes: list, by_id: dict) -> None:
    ...
    def card_cake(c: dict) -> str:
        ...
        sn = f"snip-{cid}"
        snippet = html.escape(page_embed_snippet(f"cakes/{cid}.html", base, by_id))
        return (
            f'  <div class="card">\n'
            f'    <header>\n'
...
def main():
    ...
    for cake in cakes:
        (cakes_dir / f"{cake['id']}.html").write_text(assemble_cake_page(cake), encoding="utf-8")
